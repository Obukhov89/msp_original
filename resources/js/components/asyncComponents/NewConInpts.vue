<template>
    <div class="column">
        <label class="labelFor" for="name">Введите название конкурса:</label>
        <input class="inputReg" type="text"
               @change="setStore({'key': 'nameContest', 'payload': this.nameContest})"
               v-model="nameContest" id="name" placeholder="Чудное мгновение...">

        <label class="labelFor" for="login">Дата начала:</label>
        <input class="inputReg" id="login" type="date"
               @change="setStore({'key': 'dateStart', 'payload': this.dateStart})" v-model="dateStart">
